...
function Map() {
  ...
  const [selected, setSelected] = useState({
    lat: -34.88361775757272,
    lng: -56.162513098582345,
  });
  const [direccion, setDireccion] = useState("");
  const [previous_place, setPrevious_place] = useState("");
  const [aclaraciones, setAclaraciones] = useState(
    location.state?.direccionAclaracion
  );
  ...
  const [departamento, setDepartamento] = useState(
    location.state?.direccionDepartamento
  );
  ...
  useEffect(() => {
    if (location) {
      setFirstPin(true);
      setSelected({
        lat: +location.state.direccionLat,
        lng: +location.state.direccionLng,
      });
  ...
  const addMarker = ({ latLng }) => {
    ...
    fetch(
      `https://maps.googleapis.com/maps/api/geocode/json?latlng=${newMarker.lat},${newMarker.lng}&key=AIzaSyB8FiaESvpDDrcOkwW07BVr5Z-rdumVSds`
    )
      .then((res) => res.json())
      .then((data) => {
    ...
    <>
      ...
      >
        ...
        {sucursales?.map((sucursal) => (
          <MarkerF
            key={sucursal.nombre}
            position={{
              lat: +sucursal.direccion.latitud,
              lng: +sucursal.direccion.longitud,
            }}
      ...
      <div>
        <PlacesAutocomplete
          ...
          setPrevious_place={setPrevious_place}
      ...
      <TextField
        ...
        onChange={(e) => {
          setAclaraciones(e.target.value);
        }}
      />
      <div>
        <Button
...
const PlacesAutocomplete = ({
  ...
  const handleSelect = () => {
    ...
    if (status === "OK") {
      fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?place_id=${data[0].place_id}&key=AIzaSyB8FiaESvpDDrcOkwW07BVr5Z-rdumVSds`
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    <Autocomplete
      ...
      isOptionEqualToValue={(option, direccion) =>
        option.description === direccion
      }
